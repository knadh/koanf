...
func (p *NT) Unmarshal(b []byte) (map[string]any, error) {
	r := bytes.NewReader(b)
	...
	// in an appropriate type (dict = map[string]any in this case), if necessary.
...
func (p *NT) Marshal(m map[string]any) ([]byte, error) {
